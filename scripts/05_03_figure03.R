...
df <- goog %>% filter(keyword==outc) %>% mutate(date=as.Date(date),dma=geo) %>% mutate(month=month(date)) %>% mutate(dmamonth = paste(dma,month,sep="_"))
...
n=mod$nobs
mkplot2(toplot,ylab=expression(paste("effect of 50",mu,"g day on searches for 'air quality'")),eval=50,xlab="median household income ($1000)",
        xlim=c(25000,100000),cicolor="lightblue",ylim=c(0,100),samptext = paste0("slope=",b, " per $10k (p=",round(pval,3),")"),
        hist="yes",histvar = goog$median_household_income,breaks=seq(25000,100000,2000),hist_y=0,histscale=7,histcolor = "grey")
...
pval=fixest::pvalue(mod)["smokePM:median_household_income"]
n=mod$nobs
mkplot2(toplot,eval=50,xlim=c(25000,100000),cicolor = "lightblue",ylab=expression(paste("effect of 50",mu,"g day on sentiment")),xlab="median household income ($1000)",ylim=c(-0.015,0.005),
        hist="yes",histvar=safe$median_household_income,breaks=seq(20000,140000,2000),hist_y = -0.015,histscale = 0.002,
...
pval=fixest::pvalue(mod)["smokePM:median_household_income"]
n=mod$nobs
mkplot2(toplot,ylab=expression(paste("effect of 50",mu,"g day on searches for 'air filter'")),eval=50,xlab="median household income ($1000)",
        xlim=c(30000,100000),cicolor="lightblue",ylim=c(-30,60),samptext = paste0("slope=",b, " per $10k (p=",round(pval,3),")"),
        hist="yes",histvar = goog$median_household_income,breaks=seq(30000,100000,2000),hist_y=-30,histscale=7,histcolor = "grey")
axis(1, at=seq(30,100,10)*1000,seq(30,100,10))
...
mkplot2(toplot,title="Smoke effect on % home",ylab="effect of 50ug on % completely at home",eval=50,
        xlim=c(25000,100000),ylim=c(-8,7),cicolor="lightblue",xlab="median hhold income ($1000)",hist="yes",histvar=safe$median_household_income,breaks=seq(20000,140000,2000),
