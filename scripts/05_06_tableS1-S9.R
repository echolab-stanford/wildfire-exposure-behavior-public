...
modelsummary(models,stars=F, statistic=c("({std.error})","[{p.value}]"),
             fmt=3,gof_omit = 'R2*|Log.Lik.|Std.Errors|R2|AIC|BIC',output = file.path(path_github, 'tables/S1.tex'))
...
modelsummary(hetmodels,stars=F, statistic=c("({std.error})","[{p.value}]"),
             fmt=3,gof_omit = 'R2*|Log.Lik.|Std.Errors|R2|AIC|BIC',coef_omit = "^(?!.*smokevar)",output = file.path(path_github, 'tables/S8.tex'))
