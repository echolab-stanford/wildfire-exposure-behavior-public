...
#write a wrapper to execute the clean_sensor() function and then drop some additional variables we don't care about  
run_clean_stations <- function(stationid, pa_path = pa_path){
  clean_sensor(read_csv(paste(pa_path,stationid,".csv",sep=""), col_types = "cnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnicnncc", progress = F ))
}
...
clean_sensor <- function(sensor.file, aggregate = 60, ab_tolerance_scale = 0.1, ab_tolerance_level = 10, spike.tolerance = 200, topcut = 1500){
  if(nrow(sensor.file)==0){output.data <- data.frame(Name = NA, ID = NA, Location = NA, Lon = NA, Lat = NA, year = NA, month = NA, day = NA, hour = NA, time_hours= NA, temperature = NA, humidity = NA, pressure = NA, dropped_ab_scale = NA, dropped_ab_level = NA, dropped_tso = NA, dropped_pm25 = NA,  pm25 = NA, dual_sensor = NA)}else{
    
    
    dualsensor <- TRUE
    if(sum(!is.na(sensor.file[,"PM2.5_CF_ATM_ug/m3_B"]))==0){dualsensor <- FALSE}
    ...
    output.data <- sensor.file %>% 
      ...
      dplyr::select(Name,ID, Location, Lon, Lat,  starts_with("time"), temperature = Temperature_F_A, humidity = `Humidity_%_A`, pressure = Atmos_Pres_B,starts_with("PM2.5"), ends_with("dl_A"),ends_with("dl_B") )   %>% 
    ...
    #[2]  Check for consistency across sensors in outdoor sensors and assign PM from obs with appropriate correction (varies for indoor vs outdoor)
    
    if(dualsensor == T){#new ones are outdoor dual sensor sensors. old ones may be single
    ...
    }else{ #indoor single sensor sensors
      ...
      output.data <-  output.data %>% 
        ...
        mutate(
          ...
          dropped_pm25 = pm25,
          pc0_3 = pc0_3_a ,pc0_5 = pc0_5_a, pc1_0 = pc1_0_a,pc2_5 =pc2_5_a, pc5_0 =pc5_0_a, pc10_0 =pc10_0_a,
          dropped_scale = 0, dropped_level = 0,
        )
    ...
    #[3] Filling misisng values so we have 1 obs for every 10 minutes 
    #(otherwise hard to detect outliers because adjacent obs could be different time periods)
    
    output.data <- output.data %>% ungroup() %>% tidyr::complete(time = seq(min(time), max(time), by = "10 min"), fill = list(value= NA)) 
    
    
    
    #[4]  Check for outliers that are too big to be believable
    # drop observations with extreme jumps up and back down (or down and back up) in sequential 10 minute intervals
    ...
    output.data <- output.data %>% 
      mutate(
        ...
        dropped_tso = as.numeric( ((pmlagdiff > spike.tolerance & pmleaddiff < -spike.tolerance) | (pmlagdiff < -spike.tolerance & pmleaddiff > spike.tolerance)) & (!is.na(pmlagdiff) & !is.na(pmleaddiff)) ),
    ...
    output.data <- output.data %>% 
      dplyr::select(Name, ID, Location, Lon, Lat, time, time_hours, temperature, humidity,pressure, starts_with("dropped"), pm25, pc0_3, pc0_5, pc1_0, pc2_5, pc5_0, pc10_0)
    ...
    if(aggregate==60){
      output.data <- output.data %>%
        ...
        summarise(
          ...
          dropped_missing = sum(is.na(pm25) & dropped_scale == 0 & dropped_level == 0 & dropped_tso==0 & dropped_topcut==0),
          pm25 = mean(pm25), 
          pc0_3 = mean(pc0_3),
          pc0_5 = mean(pc0_5),
          pc1_0 = mean(pc1_0),
          ...
          pc10_0 = mean(pc10_0),
          .groups = 'drop') %>% ungroup() %>% 
        tidyr::complete(time_hours = seq(min(time_hours, na.rm =T), max(time_hours, na.rm = T), by = "60 min"), fill = list(value= NA)) %>% #complete means everything gets is NA for new rows
        mutate(
        ...
        dplyr::select(Name, ID, Location, Lon, Lat, year, month, day,hour,time_hours,  temperature, humidity, pressure, dropped_ab_scale = dropped_scale, dropped_ab_level = dropped_level, dropped_tso,dropped_topcut,dropped_missing, pm25_dropped = dropped_pm25, pm25, starts_with("pc"))
    ...
    if(aggregate==10){
      ...
      output.data <- output.data %>%
        ...
        dplyr::select(Name, ID, Location, Lon, Lat, year, month, day,hour,time_hours, temperature, humidity,pressure,  dropped_ab_scale = dropped_scale, dropped_ab_level = dropped_level, dropped_tso, dropped_pm25, pm25, starts_with("pc"))
...
### add historgrams to bottom of plots
...
pm_correction <- function(pm_raw,relative_humidity,temperature, correction = "AirNow"){
  ...
  if(correction == "AirNow"){ #correction from Barkjohn et al 2020 used by AirNow: https://doi.org/10.5194/amt-2020-413
    pm_corrected <- 0.524*pm_raw - 0.0852*relative_humidity + 5.72 #equation (8) on page 11
  }
  
  if (correction == "Smoke"){ #smoke specific correction from Holder et al 2020: https://www.mdpi.com/1424-8220/20/17/4796
