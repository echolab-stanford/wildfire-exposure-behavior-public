...
dido <- read_rds(file.path(path_infiltration, "PA_monitor_level_infiltration_estimates_sfr_clean_pc.rds")) %>% filter(est_dl > 0 & est_dl <1 & !is.na(est_dl))
...
# 
# png(filename = file.path(path_github, "figures/raw/io_vs_dido_by_pm.png"), width = 1400, height = 750)
# par(mfrow = c(2,4))
# for(i in 1:7){
