...
mkplot <- function(toplot, 
                   xlab=expression(paste("smoke PM (",mu,"g/",m^{3},")")), 
                   ylab="",
                   title="",
                   xlim=c(0,100),
                   ylim=c(range(ci)),
...
mkplot(toplot,ylim=c(-10,60),xlim=c(0,50),ylab="searches for 'air quality' (index)",
       samptext = "US marketing areas, weekly 2016-20", cicolor=cll, hist="yes",
       histvar = df$smokePM,histscale = 8,hist_y = -10, xmax=50)
...
mkplot(toplot,ylab="sentiment",ylim=c(-0.06,0.02),samptext = "US counties, daily 2017-20", 
       cicolor=cll, hist="yes",histvar = df$smokePM,histscale = 0.01,hist_y = -0.06,xmax=100)
